...
# system('touch t/templates/included2.tmpl');
...
SKIP: {
  skip("Vanguard compatibility removed",1);
  $template = HTML::Template->new(                                
                                  path => 't/templates',
...
$template = HTML::Template->new(
                                path => ['t/templates/searchpath/','t/templates/'],
...
my $x;
$template = HTML::Template->new(filename => 't/templates/simple-loop.tmpl',
                                filter => {
                                           sub => sub {
                                               $x = 1;
                                               for (@{$_[0]}) {
...
$template = HTML::Template->new(filename => './t/templates/include_path/a.tmpl');
$output =  $template->output;
ok($output =~ /Bar/);
...
open(TEMPLATE, "t/templates/simple.tmpl");
...
{
    local $ENV{HTML_TEMPLATE_ROOT} = "t/templates";
    $template = HTML::Template->new(
                                    path => ['searchpath'],
                                    filename => 'three.tmpl',
                                   );
...
# test empty filename
eval { $template = $template = HTML::Template->new(path => ['t/templates'],
                                                   filename => '');
};
...
ok(!defined $template->{options}->{default_escape}, "default default_escape");
